...
async function necesidademprender(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strnecesidademprender,  codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codnecesidademprender', mssql.Int, id);
        stmt.input('strnecesidademprender', mssql.VarChar(100), strnecesidademprender)
        stmt.input('codestado', mssql.Bit, codestado)
        stmt.output('spCodeMessage', mssql.Bit);
        stmt.output('spStrMessage', mssql.VarChar(400));

        await stmt.execute('retornados.sp_necesidademprender', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodState}`, `${result.output.spMsjState}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [necesidademprender]', err))
    }
}


export default necesidademprender;
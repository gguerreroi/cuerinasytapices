...
async function tipoEventos(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strtipo, codcommcare, codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codtipo', mssql.Int, id);
        stmt.input('strtipo', mssql.VarChar(100), strtipo)
        ...
        await stmt.execute('eventos.sp_eventos_tipo', function (err, result) {
            if (err) throw err;

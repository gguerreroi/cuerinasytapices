...
async function temas(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strtema, codcommcare, codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codtema', mssql.Int, id);
        stmt.input('strtema', mssql.VarChar(100), strtema);
        ...
        await stmt.execute('eventos.sp_temas', function (err, result) {
            if (err) throw err;

...
async function evaluacionperfil(request, response) {
    ...
    const {strevaluacionperfil,  codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codevaluacionperfil', mssql.Int, id);
        stmt.input('strevaluacionperfil', mssql.VarChar(100), strevaluacionperfil)
        stmt.input('codestado', mssql.Bit, codestado)
        stmt.output('spCodeMessage', mssql.Bit);
        stmt.output('spStrMessage', mssql.VarChar(400));

        await stmt.execute('retornados.sp_evaluacionperfil', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodState}`, `${result.output.spMsjState}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [evaluacionperfil]', err))
    }
}

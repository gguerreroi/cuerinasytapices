...
async function usosuelo(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strusosuelo, codestado, codcommcare} = request.body;
    ...
    try {
        ...
        stmt.input('codusosuelo', mssql.Int, id);
        stmt.input('strusosuelo', mssql.VarChar(100), strusosuelo)
        ...
        await stmt.execute('hectareas.sp_usosuelo', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodeMessage}`, `${result.output.spStrMessage}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [usosuelo]', err))
    }
}

export default usosuelo;
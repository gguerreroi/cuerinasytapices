...
async function certificacion(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strcertificacion, strabreviatura, codestado, codcommcare} = request.body;
    ...
    try {
        ...
        stmt.input('codcertificacion', mssql.Int, id);
        stmt.input('strcertificacion', mssql.VarChar(100),  strcertificacion)
        ...
        await stmt.execute('hectareas.sp_certificacion', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodeMessage}`, `${result.output.spStrMessage}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [certificacion]', err))
    }
}

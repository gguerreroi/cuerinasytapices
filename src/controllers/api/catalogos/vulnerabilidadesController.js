...
async function vulnerabilidades(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strvulnerabilidad,  codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codvulnerabilidad', mssql.Int, id);
        stmt.input('strvulnerabilidad', mssql.VarChar(100), strvulnerabilidad)
        stmt.input('codestado', mssql.Bit, codestado)
        stmt.output('spCodeMessage', mssql.Bit);
        stmt.output('spStrMessage', mssql.VarChar(400));

        await stmt.execute('retornados.sp_vulnerabilidades', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodState}`, `${result.output.spMsjState}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [vulnerabilidades]', err))
    }
}

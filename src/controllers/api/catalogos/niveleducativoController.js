...
async function niveleducativo(request, response) {
    const {Username, Database, Password} = get_credentials(request);
    const {id} = request.params;
    const {strniveleducativo,  codestado} = request.body;
    ...
    try {
        ...
        stmt.input('codniveleducativo', mssql.Int, id);
        stmt.input('strniveleducativo', mssql.VarChar(100), strniveleducativo)
        stmt.input('codestado', mssql.Bit, codestado)
        stmt.output('spCodeMessage', mssql.Bit);
        stmt.output('spStrMessage', mssql.VarChar(400));

        await stmt.execute('retornados.sp_niveleducativo', function (err, result) {
            if (err) throw err;

            response.status(200).send(outApi(`${result.output.spCodState}`, `${result.output.spMsjState}`, result.recordset));
        })
    } catch (err) {
        response.status(500).send(outApi('500', 'Error [niveleducativo]', err))
    }
}
